...
class DB_Functions {
	...
	function resolveIssue($issueid, $emp_no){
		$date = date('Y-m-d');
		$response = array();
		$sql = "UPDATE issue SET solved = 'Yes', date_solved = '$date',solvedby = '$emp_no',
	...
	function getWaterpointsCount($staffId){
		...
		$response = array();
		$sql_a = "SELECT count(waterpoint_id) as wpcount FROM waterpoints,program,regional_office 
				  WHERE waterpoints.program_code = program.program_code 
				  AND program.regional_office_id = regional_office.regional_office_id 
				  AND regional_office.regional_office_id = $currentUser 
				  ORDER BY waterpoint_id ASC";
		$result = mysql_query($sql_a) or die(mysql_error());
	...
	function getWaterpoints($staffId){
		...
		$sql_a = "SELECT waterpoint_id, waterpoint_name, district_name, sublocation_parish, village 
				  FROM waterpoints,program,regional_office 
				  WHERE waterpoints.program_code = program.program_code 
				  AND program.regional_office_id = regional_office.regional_office_id 
				  AND regional_office.regional_office_id = $currentUser 
				  ORDER BY waterpoint_id ASC";
